...
При первом обнаружении нового завершённого матча бот получает основную
статистику и проверяет scoreboard-summary через FlareSolverr. Сообщение
отправляется в этом же запуске, без 15-минутной задержки и минутных ожиданий.
Ошибочный запрос к FlareSolverr повторяется один раз через 5 секунд; сетевые
таймауты по-прежнему действуют.

Если у отслеживаемого игрока ещё нет пары Rating/Swing, в его блоке появляется
`⏳ Rating и Swing рассчитываются`. Бот сохраняет ID отправленного сообщения,
чат, исходную статистику и время отправки в очередь внутри `last_matches.json`.
На следующих запусках cron ожидающие матчи проверяются снова, даже если игроки
уже завершили новые матчи.

Когда показатели появляются, бот редактирует исходное сообщение через
[editMessageText](https://core.telegram.org/bots/api#editmessagetext), добавляет
Rating/Swing и обновляет порядок игроков. Частичные данные сохраняются и
публикуются сразу; бот продолжает ждать остальных отслеживаемых участников
матча. Swing, равный нулю, считается готовым.

Проверки продолжаются до двух часов с момента отправки. На первом запуске
после истечения срока бот прекращает запросы Rating для этого матча и заменяет
оставшиеся пометки ожидания на `Rating и Swing недоступны`. Если Telegram
временно не принимает редактирование, бот повторит его на следующем запуске,
сохранив уже полученные показатели. Новое сообщение вместо старого не создаётся.
При `FLARESOLVERR_ENABLED=false` новые сообщения отправляются без этих полей
и без добавления в очередь.

Браузерная сессия удаляется в конце запуска и не хранится между проверками.
Очистка касается только сессий с префиксом `faceit-bot-`. Если нет ни новых
матчей, ни ожидающих обновлений, FlareSolverr не опрашивается; в журнале
остаётся только `No new matches found.`.

Строка дополнительной статистики выводится над K/D; отрицательный Swing
отмечается компактным красным индикатором:
...
Блоки игроков располагаются сверху вниз по убыванию FACEIT Rating. Если Rating
для игрока недоступен, вместо него для сортировки используется K/D.

Если Cloudflare не пройден или FlareSolverr недоступен, основная статистика
всё равно отправляется, а получение Rating/Swing переносится на следующие
запуски в пределах того же двухчасового окна.
...
Сообщения, отправленные версиями до 1.4.0, автоматически не дополняются:
их Telegram message ID не сохранялся. Очередь создаётся для новых уведомлений.
