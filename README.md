...
Новый матч обрабатывается не раньше чем через 15 минут после `finished_at`.
Если cron обнаружил его раньше, бот не отправляет сообщение, не записывает
match ID в `last_matches.json` и не обращается к FlareSolverr. На следующем
запуске этот матч будет проверен снова.

Если новых матчей нет, FlareSolverr также не опрашивается, поэтому в журнале
остаётся только строка `No new matches found.`.

После 15-минутной паузы бот создаёт уникальную браузерную сессию и запрашивает
scoreboard-summary. Если первый запрос завершается ошибкой, бот через 5 секунд
повторяет его в той же сессии. После работы бот удаляет только свою сессию.

Если scoreboard-summary уже открывается, но FACEIT ещё не рассчитал Rating и
Swing, бот не отправляет сообщение сразу. Он проверяет ту же страницу в той же
сессии до 10 раз с интервалом в одну минуту. Первая проверка выполняется сразу,
поэтому между первой и последней проверками проходит не более девяти минут. Как
только показатели появляются, бот отправляет сообщение. Если их нет и после
десятой проверки, уведомление отправляется без Rating и Swing.
...
Если обе попытки не прошли Cloudflare, FlareSolverr недоступен, ответ повреждён
или показатели не появились после 10 проверок, уведомление всё равно будет
отправлено — без Rating и Swing.
